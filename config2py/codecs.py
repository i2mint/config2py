...
try:
    ...
    def _yaml_encoder(obj: Any) -> bytes:
        ...
        return yaml.dump(obj, default_flow_style=False, allow_unicode=True).encode('utf-8')

    def _yaml_decoder(data: bytes) -> Any:
        """Decode YAML to Python object."""
        return yaml.safe_load(data.decode('utf-8'))

    register_codec('.yaml', encoder=_yaml_encoder, decoder=_yaml_decoder, dependency='pyyaml')
    register_codec('.yml', encoder=_yaml_encoder, decoder=_yaml_decoder, dependency='pyyaml')
except ImportError:
    pass
...
try:
    ...
    register_codec('.env', encoder=_env_encoder, decoder=_env_decoder, dependency='python-dotenv')
except ImportError:
